...
<template>
  <div class="mt-5">
    <table-component :table-head="thead" :total-items="carBrand.totalData" @getData="props.getData">
      <tr v-if="props.isLoading">
        <td colspan="6" class="text-center">
          <spinner class="flex justify-center w-full translate-x-[50%]" />
        </td>
      </tr>
      <tr v-if="carBrand.carBrandList.length > 0 && !props.isLoading" v-for="(item, index) in carBrand.carBrandList" :key="index">
      ...
      <tr v-if="!props.isLoading && carBrand.carBrandList.length <= 0">
        <td colspan="6">
